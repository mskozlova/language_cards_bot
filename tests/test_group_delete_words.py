...
def test_prepare_add_words(test_client, chat_id):
    ...
    for word in words[:6]:
        with utils.CommandContext(test_client, chat_id, f"🖤{word}-1 - {word}") as command:
            command.expect_any()
...
def test_group_delete_words_az(test_client, chat_id):
    ...
    with utils.CommandContext(test_client, chat_id, "/exit") as command:
        command.expect_next_prefix(texts.group_edit_finished[:40].format("abc", "delete", 3))
...
def test_group_delete_words_az_1(test_client, chat_id):
    ...
    with utils.CommandContext(test_client, chat_id, "/exit") as command:
        command.expect_next_prefix(texts.group_edit_finished[:40].format("abc", "delete", 1))
