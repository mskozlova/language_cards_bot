...
def process_exit(message, bot, pool):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        if "init_message" in data:
            utils.clear_history(
                bot, message.chat.id, data["init_message"], message.id + 1
            )
...
def process_forget_me(message, bot, pool):
    ...
    if options.delete_are_you_sure[message.text]:
        ...
        bot.send_message(
            message.chat.id, texts.forget_me_final, reply_markup=keyboards.empty
        )
    else:
        bot.send_message(
            message.chat.id, texts.cancel_short, reply_markup=keyboards.empty
        )
...
def handle_unknown(message, bot, pool):
    ...
    logger.warning(
        f"Unknown message! chat_id: {message.chat.id}, message: {message.text}"
    )
...
def handle_set_language(message, bot, pool):
    ...
    if language is not None:
        bot.send_message(
            message.chat.id,
            texts.current_language.format(language),
            reply_markup=keyboards.empty,
        )
...
def process_word_translation_stop(message, bot, pool):
    bot.delete_state(message.from_user.id, message.chat.id)
    bot.send_message(
        message.chat.id, texts.add_words_cancelled, reply_markup=keyboards.empty
    )
...
def process_word_translation_one_by_one(message, bot, pool):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        if len(data["translations"]) == len(data["words"]):  # translation is over
            ...
            bot.send_message(
                message.chat.id,
                texts.add_words_finished.format(len(data["words"])),
                reply_markup=keyboards.empty,
            )
        else:
            bot.send_message(
                message.chat.id,
                data["words"][len(data["translations"])],
                reply_markup=keyboards.empty,
            )
...
def handle_show_words(message, bot, pool):
    ...
    if len(vocab) == 0:
        bot.send_message(
            message.chat.id, texts.no_words_yet, reply_markup=keyboards.empty
        )
        return
    ...
    markup = keyboards.get_reply_keyboard(
        options.show_words_sort_options, ["/exit"], row_width=3
    )
    bot.send_message(message.chat.id, texts.choose_sorting, reply_markup=markup)

    bot.set_state(
        message.from_user.id, states.ShowWordsState.choose_sort, message.chat.id
    )
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
def process_choose_word_sort(message, bot, pool):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        if message.text not in options.show_words_sort_options:
            bot.delete_state(message.from_user.id, message.chat.id)
            bot.reply_to(
                message, texts.sorting_not_supported.format(data["original_command"])
            )
            return
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        data["batch_number"] = 0
    bot.set_state(
        message.from_user.id, states.ShowWordsState.show_words, message.chat.id
    )
...
def process_show_words_batch_next(message, bot, pool):
    ...
    words_batch = words[
        batch_number
        * constants.SHOW_WORDS_BATCH_SIZE : (batch_number + 1)
        * constants.SHOW_WORDS_BATCH_SIZE
    ]
    ...
    bot.send_message(
        message.chat.id,
        texts.word_formatted.format(
            batch_number + 1, n_pages, "\n".join(words_formatted)
        ),
        reply_markup=markup,
        parse_mode="MarkdownV2",
    )
...
def handle_show_current_language(message, bot, pool):
    ...
    if current_language is not None:
        bot.send_message(
            message.chat.id, texts.current_language.format(current_language)
        )
    else:
        utils.handle_language_not_set(message, bot)
...
def handle_show_languages(message, bot, pool):
    ...
    if len(languages) == 0:
        bot.send_message(
            message.chat.id, texts.show_languages_none, reply_markup=keyboards.empty
        )
    else:
        languages = [
...
def handle_delete_language(message, bot, pool):
    ...
    bot.set_state(
        message.from_user.id, states.DeleteLanguageState.init, message.chat.id
    )
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["language"] = language
...
def process_deleting_words(message, bot, pool):
    ...
    existing_words = db_model.get_words_from_vocab(
        pool, message.chat.id, language, words
    )
...
def process_show_group_contents(message, bot, pool):
    ...
    group_contents = sorted(
        db_model.get_group_contents(pool, group_id), key=lambda w: w["word"]
    )
...
def handle_choose_group_to_add_words(message, bot, pool):
    ...
    words_in_group = set(
        [entry["word"] for entry in db_model.get_group_contents(pool, group_id)]
    )
    ...
    bot.set_state(
        message.from_user.id, states.AddGroupWordsState.choose_sorting, message.chat.id
    )
    bot.send_message(
        message.chat.id,
        texts.choose_sorting,
        reply_markup=keyboards.get_reply_keyboard(
            options.group_add_words_sort_options, ["/exit"]
        ),
    )
...
def process_choose_sorting_to_add_words(message, bot, pool):
    ...
    if message.text not in options.group_add_words_sort_options:
        ...
        bot.reply_to(message, texts.sorting_not_supported, reply_markup=markup)
        return
    ...
    bot.set_state(
        message.from_user.id, states.AddGroupWordsState.choose_words, message.chat.id
    )
...
def process_save_group_edit(message, bot, pool):
    ...
    for entity, mask in zip(vocabulary, masks):
        ...
        if action == "delete" and mask == 0:
            chosen_words.append(entity["word"])
    n_edited_words = utils.save_words_edit_to_group(
        pool, message.chat.id, language, group_id, chosen_words, action
    )
    logger.debug(f"n_edited_words: {n_edited_words}")

    bot.reply_to(
        message,
        texts.group_edit_finished.format(
            group_name, action, n_edited_words, "\n".join(sorted(list(chosen_words)))
        ),
        reply_markup=keyboards.empty,
    )
...
def process_choose_words_batch_for_group_next(message, bot, pool):
    ...
    if batch_number * batch_size > len(vocabulary):
        ...
        bot.send_message(
            message.chat.id,
            texts.group_edit_no_more_words,
            reply_keyboard=keyboards.empty,
        )
...
def process_choose_words_batch_for_group(message, bot, pool):
    ...
    n_batches = utils.get_number_of_batches(
        constants.GROUP_ADD_WORDS_BATCH_SIZE, len(vocabulary)
    )
    ...
    markup = keyboards.get_masked_choices(
        batch, batch_mask, additional_commands=additional_commands
    )
    ...
    if is_start:
        bot.send_message(
            ...
            texts.group_edit_choose.format(
                action, group_name, batch_number + 1, n_batches
            ),
            reply_markup=markup,
        )
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data["is_start"] = False
    elif word_utils.get_word_from_group_action(message.text) not in [
        entry["word"] for entry in batch
    ]:
        logger.debug(
            f"word clicked: {word_utils.get_word_from_group_action(message.text)}"
        )
        bot.send_message(
            message.chat.id, texts.group_edit_unknown_word, reply_markup=markup
        )
    else:
        ...
        masks[
            batch_number * constants.GROUP_ADD_WORDS_BATCH_SIZE + word_idx
        ] = batch_mask[word_idx]
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data["masks"] = masks
...
def handle_choose_group_to_delete_words(message, bot, pool):
    ...
    bot.set_state(
        message.from_user.id,
        states.DeleteGroupWordsState.choose_sorting,
        message.chat.id,
    )
    bot.send_message(
        message.chat.id,
...
def process_choose_sorting_to_delete_words(message, bot, pool):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["vocabulary"] = vocabulary
    bot.set_state(
        message.from_user.id, states.DeleteGroupWordsState.choose_words, message.chat.id
    )
    process_choose_words_batch_for_group(message, bot, pool)
...
def process_group_deletion_check_sure(message, bot, pool):
    ...
    bot.set_state(
        message.from_user.id, states.DeleteGroupState.are_you_sure, message.chat.id
    )
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
...
def process_group_deletion(message, bot, pool):
    ...
    if not options.delete_are_you_sure[message.text]:
        bot.send_message(
            message.chat.id, texts.delete_group_cancel, reply_markup=keyboards.empty
        )
        return
...
def handle_train(message, bot, pool):
    ...
    reply_message = bot.send_message(
        message.chat.id, texts.training_init, reply_markup=markup
    )

    bot.set_state(
        message.from_user.id, states.TrainState.choose_strategy, message.chat.id
    )
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["language"] = language
...
def process_choose_strategy(message, bot, pool):
    if message.text not in options.train_strategy_options:
        markup = keyboards.get_reply_keyboard(
            options.train_strategy_options, ["/cancel"]
        )
...
def process_choose_group_for_training(message, bot, pool):
    ...
    if len(groups) == 0:
        bot.reply_to(
            message, texts.no_such_group.format("/train"), reply_markup=keyboards.empty
        )
        utils.suggest_group_choices(message, bot, pool, states.TrainState.choose_group)
        return
...
def process_choose_direction(message, bot, pool):
    ...
    bot.set_state(
        message.from_user.id, states.TrainState.choose_duration, message.chat.id
    )
    markup = keyboards.get_reply_keyboard(options.train_duration_options, ["/cancel"])
    bot.send_message(message.chat.id, texts.training_duration, reply_markup=markup)
...
def process_choose_duration(message, bot, pool):
    if (
        ...
        and message.text not in options.train_duration_options
    ):
        markup = keyboards.get_reply_keyboard(
            options.train_duration_options, ["/cancel"]
        )
    ...
    bot.send_message(message.chat.id, texts.training_hints, reply_markup=markup)

    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        data["duration"] = (
            int(message.text) if message.text.isdigit() else config.TRAIN_MAX_N_WORDS
        )
...
def process_choose_hints(message, bot, pool):
    ...
    if strategy != "group":
        db_model.create_training_session(
            pool, message.chat.id, session_id, strategy, language, direction, duration
        )
    else:
        db_model.create_group_training_session(
            pool,
            message.chat.id,
            session_id,
            strategy,
    ...
    if len(words) == 0:
        bot.delete_state(message.from_user.id, message.chat.id)
        bot.send_message(
            message.chat.id, texts.training_no_words_found, reply_markup=keyboards.empty
        )
        return
    ...
    train_message = bot.send_message(
        ...
        texts.training_start.format(
            ...
            direction,
            hints,
            texts.training_start_group.format(group_name)
            if strategy == "group"
            else "",
        ),
        reply_markup=keyboards.empty,
    ...
    if len(words) < duration and duration != config.TRAIN_MAX_N_WORDS:
        bot.send_message(
            message.chat.id, texts.training_fewer_words, reply_markup=keyboards.empty
        )
...
def handle_train_step_stop(message, bot, pool):
    bot.delete_state(message.from_user.id, message.chat.id)
    bot.send_message(
        message.chat.id, texts.training_stopped, reply_markup=keyboards.empty
    )
...
def handle_train_step(message, bot, pool):
    ...
    logger.debug(
        f"step: {step}, words: {words}, scores: {scores}, hints: {hints}, direction: {direction}"
    )

    if step != 0:  # not a first iteration
        word = words[step - 1]
        is_correct = word_utils.compare_user_input_with_db(
            message.text, word, hints, direction
        )
    ...
    if step == len(words):  # training complete
        if hints == "no hints":
            ...
            db_model.update_final_scores(
                pool, message.chat.id, session_id, language, direction
            )
        else:
