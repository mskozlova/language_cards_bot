...
def get_show_words_handlers():
    return [
        ...
        Handler(
            handlers.process_choose_word_sort,
            state=bot_states.ShowWordsState.choose_sort,
        ),
        Handler(
            handlers.process_exit,
        ...
        Handler(
            handlers.process_show_words_batch_next,
            state=bot_states.ShowWordsState.show_words,
            commands=["next"],
        ),
        Handler(
            handlers.process_show_words_batch_unknown,
            state=bot_states.ShowWordsState.show_words,
        ),
    ]
...
def get_show_languages_handlers():
    return [
        Handler(
            handlers.handle_show_current_language, commands=["show_current_language"]
        ),
...
def get_delete_language_handlers():
    return [
        ...
        Handler(
            handlers.process_delete_language, state=bot_states.DeleteLanguageState.init
        ),
    ]
...
def get_delete_words_handlers():
    return [
        ...
        Handler(
            handlers.process_deleting_words, state=bot_states.DeleteWordsState.init
        ),
    ]
...
def get_create_group_handlers():
    return [
        ...
        Handler(
            handlers.process_group_creation, state=bot_states.CreateGroupState.init
        ),
    ]
...
def get_delete_group_handlers():
    return [
        ...
        Handler(
            handlers.process_group_deletion_check_sure,
            state=bot_states.DeleteGroupState.select_group,
        ),
        Handler(
            handlers.process_group_deletion,
...
def get_group_add_words_handlers():
    return [
        ...
        Handler(
            handlers.process_exit,
            commands=["exit"],
            state=bot_states.AddGroupWordsState.choose_group,
        ),
        Handler(
            handlers.handle_choose_group_to_add_words,
            state=bot_states.AddGroupWordsState.choose_group,
        ),
        Handler(
            handlers.process_exit,
            commands=["exit"],
            state=bot_states.AddGroupWordsState.choose_sorting,
        ),
        Handler(
            handlers.process_choose_sorting_to_add_words,
            state=bot_states.AddGroupWordsState.choose_sorting,
        ),
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.AddGroupWordsState.choose_words,
        ),
        Handler(
            handlers.process_save_group_edit,
            commands=["exit"],
            state=bot_states.AddGroupWordsState.choose_words,
        ),
        Handler(
            handlers.process_choose_words_batch_for_group_next,
            commands=["next"],
            state=bot_states.AddGroupWordsState.choose_words,
        ),
        Handler(
            handlers.process_choose_words_batch_for_group,
            state=bot_states.AddGroupWordsState.choose_words,
        ),
    ]
...
def get_group_delete_words_handlers():
    return [
        ...
        Handler(
            handlers.process_exit,
            commands=["exit"],
            state=bot_states.DeleteGroupWordsState.choose_group,
        ),
        Handler(
            handlers.handle_choose_group_to_delete_words,
            state=bot_states.DeleteGroupWordsState.choose_group,
        ),
        Handler(
            handlers.process_exit,
            commands=["exit"],
            state=bot_states.DeleteGroupWordsState.choose_sorting,
        ),
        Handler(
            handlers.process_choose_sorting_to_delete_words,
            state=bot_states.DeleteGroupWordsState.choose_sorting,
        ),
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.DeleteGroupWordsState.choose_words,
        ),
        Handler(
            handlers.process_save_group_edit,
            commands=["exit"],
...
def get_train_handlers():
    return [
        ...
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.TrainState.choose_strategy,
        ),
        Handler(
            handlers.process_choose_strategy,
            state=bot_states.TrainState.choose_strategy,
        ),
        Handler(
            handlers.process_exit,
            commands=["exit"],
            state=bot_states.TrainState.choose_group,
        ),
        Handler(
            handlers.process_choose_group_for_training,
            state=bot_states.TrainState.choose_group,
        ),
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.TrainState.choose_direction,
        ),
        Handler(
            handlers.process_choose_direction,
            state=bot_states.TrainState.choose_direction,
        ),
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.TrainState.choose_duration,
        ),
        Handler(
            handlers.process_choose_duration,
            state=bot_states.TrainState.choose_duration,
        ),
        Handler(
            handlers.process_cancel,
            commands=["cancel"],
            state=bot_states.TrainState.choose_hints,
        ),
        Handler(
            handlers.process_choose_hints, state=bot_states.TrainState.choose_hints
        ),
        Handler(
            handlers.handle_train_step_stop,
            commands=["stop"],
            state=bot_states.TrainState.train,
        ),
...
def get_test_handlers():
    return [Handler(test_handlers.handle_clear_db, commands=["clear_db"])]
